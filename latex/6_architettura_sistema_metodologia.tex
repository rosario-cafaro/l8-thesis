...
\begin{enumerate}
    ...
    \item \textbf{Modulo di codifica quantistica}: trasforma i vettori di feature preprocessati in circuiti quantistici parametrici (feature map), secondo le modalità descritte nella Sezione~\ref{sec:circuiti_quantistici}.
    \item \textbf{Modulo di addestramento}: comprende sia gli algoritmi classici di Scikit-learn, utilizzati come baseline, sia i modelli quantistici QSVC e VQC implementati tramite qiskit-machine-learning.
    \item \textbf{Modulo di esecuzione}: gestisce l'invio dei circuiti al backend selezionato, che può essere un simulatore locale privo di rumore, un simulatore con modello di rumore, oppure un dispositivo quantistico reale accessibile tramite IBM Quantum Platform.
    \item \textbf{Modulo di valutazione}: calcola le metriche di performance (Sezione~\ref{sec:protocollo_sperimentale}) e produce i risultati comparativi tra i modelli classici e quantistici.
\end{enumerate}

Il flusso dei dati attraversa questi moduli in sequenza, ma l'architettura consente di eseguire ciascuna fase in maniera indipendente e di salvare i risultati intermedi (ad esempio i dati normalizzati e ridotti dimensionalmente), evitando di dover ripetere l'intera pipeline per ogni configurazione testata.
...
Questa libreria si basa sui concetti fondamentali presentati nel Qiskit SDK e permette di costruire modelli completamente quantistici senza dover gestire manualmente ogni singolo dettaglio a livello di porte logiche quantistiche, pur mantenendo un controllo esplicito sulla struttura del circuito.

\subsection{Ambiente Jupyter e workflow di sviluppo}
Il workflow di sviluppo è strutturato in modo da permettere un facile riutilizzo del codice, una documentazione chiara e un confronto sistematico tra i modelli quantistici e classici. Le fasi principali, che rispecchiano i moduli architetturali descritti nella Sezione~\ref{sec:architettura_generale}, sono:
...
\begin{enumerate}
    ...
    \item \textbf{Valutazione}: misurazione delle performance mediante le metriche descritte nella Sezione~\ref{sec:protocollo_sperimentale} e confronto sistematico con i modelli classici.
    \item \textbf{Analisi dei risultati}: studio delle prestazioni di ciascun modello e discussione dei relativi vantaggi e limiti.
\end{enumerate}

Questo approccio modulare permette di testare rapidamente diverse configurazioni per identificare quelle più efficaci in base alle caratteristiche specifiche dei dati e del problema da risolvere, senza dover riscrivere le fasi comuni della pipeline.
...
La scelta congiunta dei tre dataset è motivata dal fatto che, pur condividendo il vincolo di feature numeriche e problemi di classificazione ben definiti, essi coprono uno spettro di condizioni sperimentali differenti: classificazione binaria e multiclasse, dimensionalità originaria bassa, media e alta, e domini applicativi diversi (diagnostica medica, chimica analitica, visione artificiale).

\subsection{Preprocessing e normalizzazione}
Le operazioni di preprocessing sono essenziali per adattare i dataset alle esigenze specifiche del machine learning quantistico. La normalizzazione dei dati è in particolare necessaria per garantire la stabilità dell'addestramento e la corretta interpretazione dei valori delle feature come angoli di rotazione nei circuiti quantistici.
...
Questa normalizzazione è necessaria perché le feature map quantistiche utilizzate impiegano porte di rotazione (RX, RY, RZ) i cui angoli dipendono dai valori delle feature in ingresso; un intervallo limitato e uniforme è pertanto opportuno per evitare rotazioni eccessive che comprometterebbero la distinguibilità degli stati quantistici codificati. La scelta di questo approccio, rispetto ad alternative come la standardizzazione (\textit{z-score}), è motivata da:
\begin{itemize}
    \item facilità di interpretazione e implementazione;
    \item compatibilità diretta con il formalismo delle porte di rotazione quantistiche, che operano naturalmente su intervalli angolari limitati;
    \item maggiore stabilità dell'addestramento osservata empiricamente nelle fasi preliminari di sperimentazione;
    \item rappresentazione uniforme dei dati, indipendentemente dalla scala originaria delle singole feature.
\end{itemize}
...
\label{sec:pca}
Poiché il numero di qubit disponibili, sia in simulazione sia su hardware reale, è limitato, è necessario ridurre la dimensionalità dei dataset con dimensionalità originaria elevata mediante l'Analisi delle Componenti Principali (\textit{Principal Component Analysis}, PCA)\footnote{Jolliffe, I. T. (2002). \textit{Principal Component Analysis} (2nd ed.). Springer Series in Statistics. Springer.}. Questa riduzione è essenziale per:
\begin{itemize}
    \item adattare i dati alla struttura dei circuiti quantistici, in cui ogni feature è tipicamente codificata su un qubit dedicato;
    \item mantenere i pattern essenziali presenti nei dati, minimizzando la perdita di informazione discriminante tra le classi;
    \item ridurre i tempi di calcolo necessari per l'addestramento, sia in simulazione sia in esecuzione reale;
    \item combinare le feature originarie in un numero ridotto di componenti rappresentabili su un numero contenuto di qubit.
\end{itemize}

Il processo di riduzione si basa sull'analisi della varianza spiegata da ciascuna componente principale. Vengono selezionate le componenti che rappresentano complessivamente una percentuale significativa della varianza totale del dataset, individuata tramite l'analisi del grafico della varianza cumulata (\textit{scree plot}).
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{Varianza spiegata cumulata in funzione del numero di componenti principali per i dataset Breast Cancer Wisconsin, Wine e Digits. I marcatori cerchiati indicano il numero di componenti effettivamente selezionato per ciascun dataset (rispettivamente 4, 5 e 6).}
...
\begin{itemize}
    \item \textbf{QSVC (Quantum Support Vector Classifier)}: utilizza la feature map per calcolare un kernel quantistico, ossia una misura di similarità tra coppie di campioni definita come sovrapposizione tra i corrispondenti stati quantistici codificati. Il kernel quantistico così ottenuto viene successivamente impiegato all'interno di un classificatore a support vector machine classico, secondo l'approccio proposto da Rebentrost et al.\footnote{Rebentrost, P., Mohseni, M., \& Lloyd, S. (2014). Quantum support vector machine for big data classification. \textit{Physical Review Letters}, 113(13), 130503. \url{https://doi.org/10.1103/PhysRevLett.113.130503}.} e da Havlíček et al.\footnote{Havlíček, V., Córcoles, A. D., Temme, K., Harrow, A. W., Kandala, A., Chow, J. M., \& Gambetta, J. M. (2019). Supervised learning with quantum-enhanced feature spaces. \textit{Nature}, 567(7747), 209--212. \url{https://doi.org/10.1038/s41586-019-0980-2}.}. In questo schema, la parte quantistica del modello è limitata al calcolo del kernel, mentre l'ottimizzazione del confine di decisione avviene con metodi classici consolidati.
    \item \textbf{VQC (Variational Quantum Classifier)}: applica in sequenza la feature map e l'ansatz variazionale, misura lo stato risultante e interpreta l'esito della misura come predizione di classe. I parametri dell'ansatz sono aggiornati iterativamente da un ottimizzatore classico, secondo lo schema ibrido quantistico-classico tipico degli algoritmi variazionali. A differenza del QSVC, in questo caso l'intero processo di apprendimento, e non solo il calcolo del kernel, avviene tramite il circuito quantistico.
\end{itemize}
...
\texttt{StatevectorSampler} è una classe della libreria Qiskit che simula il comportamento di un circuito quantistico calcolando il vettore di stato completo per produrre campioni di misura, eseguendo simulazioni esatte prive di rumore. Questo approccio è particolarmente utile per:
\begin{itemize}
    \item test su un numero contenuto di qubit (indicativamente fino a 8-10 qubit, oltre i quali la simulazione statevector diventa rapidamente onerosa);
    \item analisi dettagliata e comprensione del comportamento ideale del modello;
    \item verifica della correttezza logica dei circuiti quantistici implementati;
    \item validazione delle implementazioni intermedie, prima di procedere a simulazioni più realistiche o all'esecuzione su hardware reale.
\end{itemize}
...
Nel presente lavoro, l'utilizzo di Qiskit Aer con modelli di rumore configurabili (ottenuti, ad esempio, a partire dalle caratteristiche di calibrazione di dispositivi reali) permette di testare i modelli quantistici in condizioni intermedie tra la simulazione ideale e l'esecuzione su hardware reale, valutando sia la performance ideale sia una stima della performance realistica prima di impegnare risorse computazionali sul hardware effettivo.
...
\begin{itemize}
    ...
    \item \textbf{Qualità del hardware}: una maggiore fedeltà del dispositivo, misurata ad esempio tramite i parametri di calibrazione pubblicati da IBM, tende a produrre risultati più vicini alla simulazione ideale;
