...
Quando si lavora con l'apprendimento non supervisionato, l'obiettivo principale è
mappare i dati senza avere a disposizione una ``risposta corretta'' o un'etichetta
di riferimento. In pratica, il dataset è composto unicamente dai vettori di input
\begin{equation}
\mathbf{x}_i \in \mathbb{R}^d \quad (\text{per } i = 1, \dots, n)
\end{equation}
che definiscono lo spazio delle caratteristiche. Non essendoci una guida esterna o una \textit{ground truth} a fare da bussola, l'algoritmo deve muoversi in autonomia: esplora la geometria dei dati per far emergere da solo pattern nascosti, densità di probabilità o raggruppamenti spontanei.
Si tratta di un approccio decisamente esplorativo, fondamentale nelle fasi iniziali di analisi o per ridurre la dimensionalità delle informazioni.
...
\begin{itemize}
    ...
    \item \textbf{Riduzione della Dimensionalità:} Si occupa della proiezione dei dati da uno spazio ad alta dimensionalità $d$ a uno spazio a bassa dimensionalità $k$ (con $k \ll d$). Questo tipo di approccio è essenziale per disinnescare la cosiddetta ``maledizione della dimensionalità'' (\textit{curse of dimensionality}), un problema che altrimenti renderebbe i dati intrattabili.
    Riducendo le variabili, non solo si alleggerisce il carico computazionale, ma si riesce a dare una forma visiva a strutture altrimenti invisibili, senza però distruggere le informazioni chiave del dataset.
    Sul fronte lineare, lo strumento d'elezione resta l'Analisi delle Componenti Principali (\textit{PCA}), che si basa sulla massimizzazione della varianza lungo assi ortogonali.
    Quando però la geometria si complica e ci si scontra con varietà non lineari (\textit{manifolds}), entrano in gioco algoritmi più raffinati come t-SNE e UMAP, pensati proprio per mappare il problema rispettando le relazioni di vicinanza locali e globali.
\end{itemize}
...
Gli algoritmi basati sulla distanza (come SVM, $k$-NN) e gli algoritmi basati sull'ottimizzazione (come la Regressione Logistica) sono estremamente sensibili alle incongruenze di scala nelle variabili. Pertanto la scelta del metodo di ridimensionamento diventa estremamente importante in quanto una scelta sbagliata può effettivamente influenzare l'efficienza e la coerenza dell'algoritmo.
L'encoding diventa distorto per gli algoritmi quantistici nel caso di un ridimensionamento sbagliato.
...
La curva ROC (\textit{Receiver Operating Characteristic}) descrive il comportamento del classificatore al variare della soglia decisionale $\tau$, tracciando il True Positive Rate (TPR) rispetto al False Positive Rate (FPR). Integrando la curva ROC, la misura nota come \textbf{ROC-AUC} ci fornisce un valore assoluto che indica la potenza discriminante del modello indipendentemente dalla soglia selezionata. È importante menzionare che a differenza dei modelli classici dove la soglia $\tau$ è molto flessibile, nel caso dei modelli quantistici essa deve essere mappata su un continuum attraverso le ampiezze probabilistiche degli stati quantistici.
